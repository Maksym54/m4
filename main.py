...
class GoItFramework(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        match route.path:
            ...
            case _:
                ...
                if file.exists():
                    self.send_static(file.name, 200)
                else:
    ...
    def do_post(self):
        ...
        client_socket.sendto(data, (socket.AF_INET, socket.SOCK_DGRAM))
        client_socket.close()
...
def save_data_from_form(data):
    ...
    print(parse_data)
    try:
        parse_dict = {key: value for key, value in [el.split('=') for el in parse_data.split('/omega')]}
        print(parse_dict)
...
def run_socket_server(host,port):
    ...
    try:
        while True:
            msg = server_socket.recvfrom(BUFFER_SIZE)
